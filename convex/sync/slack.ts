...
export const syncMessages = action({
  ...
  handler: async (ctx, args) => {
    ...
    // This avoids re-processing the entire history on every cron tick.
    const latestMsg = await ctx.runQuery(api.messages.getLatestForIdentity, {
      identityId: args.identityId,
      platform: "slack",
    });
    // Slack `oldest` is a Unix timestamp string (seconds, not ms). Add 1ms to
    // exclude the message we already have so we only get strictly newer ones.
    const oldest = latestMsg
      ? ((latestMsg.timestamp + 1) / 1000).toString()
      : undefined;

    if (oldest) {
      console.log(
        `Slack syncMessages: incremental sync from ts=${oldest} for identity=${args.identityId}`
      );
    }
