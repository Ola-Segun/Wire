...
export const analyzeMessage = action({
  ...
  handler: async (ctx, args) => {
    ...
    //
    // Fetching the last 5 inbound messages from this client and any existing
    // conversation summary gives the AI multi-turn context rather than
    // analysing each message in isolation. This dramatically improves:
    //   - Churn risk detection (tone shift patterns vs. single-message signals)
    //   - Hidden request identification (recurring asks across messages)
    //   - Priority scoring (escalation patterns are visible)
    //   - Action extraction (follow-ups on previously mentioned items)
    //
    // Cost: +2 Convex reads per message (getRecentInboundByClient + summary).
    // No extra LLM tokens beyond the appended context block.

    let conversationContext = "";

    // 1. Recent message history (up to 5 prior inbound messages)
    const recentHistory: Array<Record<string, any>> = await ctx.runQuery(
      internal.messages.getRecentInboundByClient,
      { clientId: message.clientId, limit: 6 }
    );
    // Exclude the current message (it's already fetched above) and get last 5
    ...
    if (contextMessages.length > 0) {
      const historyLines = [...contextMessages]
        ...
        .map((m) => {
          const sentimentHint = m.aiMetadata?.sentiment
            ? ` [${m.aiMetadata.sentiment}]`
            : "";
          const truncated = m.text.length > 200
            ? m.text.slice(0, 200) + "…"
            : m.text;
          return `- "${truncated}"${sentimentHint}`;
        })
        .join("\n");
      conversationContext += `\nRecent messages from this client (oldest to newest):\n${historyLines}`;
    }

    // 2. Conversation summary (cached, generated by thread_summarizer)
    if (message.conversationId) {
      ...
      if (summary) {
        ...
        if (summary.openCommitments > 0) {
          conversationContext += ` (${summary.openCommitments} open commitment${summary.openCommitments > 1 ? "s" : ""})`;
        }
        if (summary.toneShift) {
          conversationContext += ` Tone shift detected: ${summary.toneShift}.`;
        }
      }
    }

    // 3. Aggregated client intelligence (computed by health cron, zero LLM cost)
    const intel = client?.intelligence as Record<string, any> | undefined | null;
    if (intel) {
      const intelParts: string[] = [];
      if (intel.sentimentTrend && intel.sentimentTrend !== "stable")
        intelParts.push(`relationship trend: ${intel.sentimentTrend}`);
      if (intel.aggregateChurnRisk && intel.aggregateChurnRisk !== "none")
        intelParts.push(`aggregate churn risk: ${intel.aggregateChurnRisk}`);
      if (intel.dominantPhase)
        intelParts.push(`dominant project phase: ${intel.dominantPhase}`);
      if (intelParts.length > 0)
        conversationContext += `\nClient relationship signals: ${intelParts.join(", ")}.`;
    }
...
Platform: ${message.platform}${conversationContext}
