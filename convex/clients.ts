...
// Resolve current user from auth identity — shared across mutations
...
export const createFromIdentity = mutation({
  ...
  handler: async (ctx, args) => {
    // Auth guard: verify caller owns the identity
    const user = await resolveUser(ctx);
    if (!user) throw new Error("Not authenticated");

    const identity = await ctx.db.get(args.identityId);
    if (!identity) throw new Error("Identity not found");
    if (identity.userId !== user._id) throw new Error("Unauthorized");
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    // Auth guard: verify caller owns the client
    const user = await resolveUser(ctx);
    if (!user) throw new Error("Not authenticated");
    ...
    if (client.userId !== user._id) throw new Error("Unauthorized");
...
// Internal query: fetch a single client by ID without auth context.
// Used by health.calculateForClient when called from cron — no auth available.
...
// Internal query: fetch all active clients for a userId without auth context.
// Used by the health recalculation cron which runs without a logged-in user.
export const getActiveByUserInternal = internalQuery({
