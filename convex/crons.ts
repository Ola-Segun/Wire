...
// DEV: 15 minutes — reduces bandwidth/function calls during testing.
// PROD: change back to { minutes: 3 } for near-real-time delivery.
crons.interval(
  "sync-all-messages",
  { minutes: 15 },
...
// Recalculate relationship health scores for all clients every 4 hours.
// Uses internalAction so it works without auth context.
crons.interval(
  "recalculate-relationship-health",
  { hours: 4 },
  internal.health.recalculateAll
);
...
// ============================================
// SKILLS — Cron-triggered skill checks
// ============================================

// Run cron skills (commitment watchdog, ghosting detector, payment sentinel)
// every 4 hours. Cost: 0 Claude calls — pure DB queries.
crons.interval(
  "run-cron-skills",
  { hours: 4 },
  internal.skillDispatcher.runCronSkills
);

// Clean up expired skill outputs daily to keep the DB lean.
crons.interval(
  "cleanup-expired-skill-outputs",
  { hours: 24 },
  internal.skills.cleanupExpired
);
...
// Generate daily briefings for all users at 7am UTC.
// Cost: 1 Haiku call per user (~$0.0001/user/day).
// Deduplication in generateForUser prevents double-runs on redeploys.
crons.cron(
  "run-daily-briefings",
  "0 7 * * *",
