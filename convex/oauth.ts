...
// AES-256-GCM before storing and decrypted on retrieval. Implementation:
// 1. Create src/lib/crypto.ts with encrypt/decrypt using ENCRYPTION_KEY env var
// 2. Encrypt in storeTokens mutation before db.insert/db.patch
// 3. Decrypt in getTokens query before returning
// 4. Add ENCRYPTION_KEY to .env.example (32-byte hex key)
// See: implementation_plan.md Phase 1 > Token Encryption
...
// Find a WhatsApp Business connection by Phone Number ID.
// Used by the Meta webhook handler to route incoming messages to the correct Wire user.
// platformUserId stores the Meta Phone Number ID (e.g. "123456789012345") for WhatsApp.
// A full scan is acceptable — WhatsApp-connected users are a small subset of total users.
export const findWhatsAppConnectionByPhoneNumberId = query({
  args: { phoneNumberId: v.string() },
  handler: async (ctx, args) => {
    const tokens = await ctx.db.query("oauth_tokens").collect();
    const match = tokens.find(
      (t) => t.platform === "whatsapp" && t.platformUserId === args.phoneNumberId
    );
    if (!match) return null;
    return {
      userId: match.userId,
      accessToken: match.accessToken,
      phoneNumberId: match.platformUserId!,
    };
  },
});
