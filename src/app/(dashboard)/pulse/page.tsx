...
export default function PulsePage() {
  ...
  // Initial page of 50; user can load more via the list footer button.
  const [cursor, setCursor] = useState<string | null | undefined>(undefined);
  const [loadedMessages, setLoadedMessages] = useState<any[]>([]);
  const page = useQuery(api.messages.getAll, { limit: 50, cursor: cursor ?? undefined });

  // Accumulate pages as cursor advances
  const prevCursorRef = useRef<string | null | undefined>(undefined);
  useMemo(() => {
    if (!page) return;
    if (cursor === undefined || cursor !== prevCursorRef.current) {
      // First page or cursor reset: replace
      if (cursor === undefined || cursor === null) {
        setLoadedMessages(page.messages);
      } else {
        // Subsequent page: append
        setLoadedMessages((prev) => {
          const existingIds = new Set(prev.map((m: any) => m._id));
          const newOnes = page.messages.filter((m: any) => !existingIds.has(m._id));
          return [...prev, ...newOnes];
        });
      }
  ...
  const allMessages = { messages: loadedMessages, hasMore: page?.hasMore ?? false, nextCursor: page?.nextCursor };
  ...
  const handleSelectClient = useCallback((id: string | null) => {
    ...
    setSelectedMessageId(null);
    setShowThread(false);
    // Reset pagination cursor when switching client filter
    setCursor(undefined);
  }, []);
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto scrollbar-thin">
                    {filteredMessages.length === 0 ? (
                      ...
                      {/* Load more — only shown when not filtered by client */}
                      {!selectedClientId && allMessages.hasMore && (
                        <button
                          onClick={() => setCursor(allMessages.nextCursor)}
                          className="w-full py-3 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary/40 transition-colors border-t border-border/40"
                        >
                          Load more messages
                        </button>
                      )}
