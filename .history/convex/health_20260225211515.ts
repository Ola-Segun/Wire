...
export const calculateForClient = action({
  ...
  handler: async (ctx, args): Promise<{ health: number }> => {
    // Fetch client and recent messages via queries
    const client = await ctx.runQuery(api.clients.get, { id: args.clientId });
    if (!client) throw new Error("Client not found");

    const result = await ctx.runQuery(api.messages.getByClient, {
      clientId: args.clientId,
      limit: 100,
    });

    // getByClient returns { messages, nextCursor, hasMore }
    const messages: HealthMessage[] = result.messages;
...
export const calculateForAllClients = action({
  ...
  handler: async (ctx, args): Promise<{ calculated: number }> => {
    const clients = await ctx.runQuery(api.clients.getByUser, {});
