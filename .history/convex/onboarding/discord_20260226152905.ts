...
// ============================================
//
// Discovers Discord users from guilds where the bot is present.
// Uses the BOT token exclusively (never expires) rather than the
// user's OAuth token (expires after ~7 days).
// Follows the same transient pattern as slack.discoverNewUsers —
// no DB writes until the user confirms.

const DISCORD_API = "https://discord.com/api/v10";

// Discover new Discord users from bot's guilds not yet tracked.
...
export const discoverNewUsers = action({
  ...
  handler: async (
    ...
    // Verify Discord is connected (we need the user's platformUserId to exclude self)
    ...
    // 1. Get guilds the BOT belongs to (bot token never expires)
    ...
    if (guildsRes.status === 429) {
      const retryAfter = parseFloat(guildsRes.headers.get("retry-after") || "5");
      console.log(`Discord discover: rate limited, retrying after ${retryAfter}s`);
      await new Promise((r) => setTimeout(r, retryAfter * 1000));
      guildsRes = await fetch(`${DISCORD_API}/users/@me/guilds`, { headers });
    }

    if (!guildsRes.ok) {
      const errBody = await guildsRes.text();
      console.error(
        `Discord discover: failed to fetch bot guilds: ${guildsRes.status} — ${errBody}`
      );
      throw new Error(
        `Failed to fetch Discord guilds (${guildsRes.status}). Check DISCORD_BOT_TOKEN.`
      );
    }

    const guilds: Array<{ id: string; name: string }> = await guildsRes.json();
    console.log(
      `Discord discover: bot is in ${guilds.length} guild(s): ${guilds.map((g) => g.name).join(", ")}`
    );
    ...
    for (const guild of guilds.slice(0, 5)) {
      ...
      try {
        ...
        if (!membersRes.ok) {
          ...
          console.error(
            `Discord discover: guild ${guild.name} (${guild.id}) members failed: ${membersRes.status} — ${errText}`
          );
          continue;
        }
