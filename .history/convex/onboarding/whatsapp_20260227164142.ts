...
export const discoverNewUsers = action({
  ...
  handler: async (
    ...
    // Exclude the user's own phone number (stored in oauth_tokens)
    const selfPhone = tokens.platformUserId;

    // Return identities that are NOT yet linked to a client and not self
    const unlinked = identities
      .filter((id) => !id.clientId && id.platformUserId !== selfPhone)
      .map((id) => ({
...
export const addContactByPhone = action({
  ...
  handler: async (ctx, args): Promise<{ identityId: string; alreadyExists: boolean }> => {
    ...
    if (found) {
      return { identityId: found._id as string, alreadyExists: true };
    }

    // Create new identity
    const now = Date.now();
    const newId: string = await ctx.runMutation(api.identities.create, {
      userId: args.userId,
      platform: "whatsapp",
    ...
    return { identityId: newId, alreadyExists: false };
  },
});
