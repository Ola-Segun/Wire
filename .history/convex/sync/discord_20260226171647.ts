...
export const syncMessages = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      const headers = getDiscordHeaders();
      const url = `${DISCORD_API}/channels/${identity.dmChannelId}/messages?limit=100`;
      console.log(`Discord syncMessages: fetching from channel=${identity.dmChannelId}`);

      const messagesResponse = await fetch(url, { headers });

      if (!messagesResponse.ok) {
        const errBody = await messagesResponse.text();
        console.error(
          `Discord syncMessages: failed to fetch messages: ${messagesResponse.status} — ${errBody}`
        );
        return { synced: 0 };
      }
      ...
      console.log(
        `Discord syncMessages: got ${messages.length} raw messages from channel=${identity.dmChannelId}`
      );

      // Log first few messages for debugging
      for (const m of messages.slice(0, 3)) {
        console.log(
          `Discord syncMessages: sample msg id=${m.id} ` +
          `author=${m.author?.username} bot=${m.author?.bot} ` +
      ...
      console.log(`Discord syncMessages: wireUserDiscordId=${wireUserDiscordId}`);

      for (const msg of messages) {
        // Skip bot messages but allow empty content (attachments-only)
