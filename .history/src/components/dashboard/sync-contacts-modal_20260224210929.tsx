...
import { Loader2, Mail, MessageSquare, Search, X } from "lucide-react";
...
type Contact = GmailContact | SlackUser;
...
export function SyncContactsModal({
  ...
  const getId = (c: Contact): string =>
    platform === "gmail"
      ...
      : (c as SlackUser).platformUserId;

  const getLabel = (c: Contact): string =>
    platform === "gmail"
      ? (c as GmailContact).name
      : (c as SlackUser).displayName;
  ...
  const PlatformIcon = platform === "gmail" ? Mail : MessageSquare;
  const platformLabel = platform === "gmail" ? "Gmail" : "Slack";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="surface-raised rounded-xl w-full max-w-lg mx-4 max-h-[85vh] flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border/20">
          <div className="flex items-center gap-2">
            <PlatformIcon
              className={`h-4 w-4 ${platform === "gmail" ? "text-urgent" : "text-chart-4"}`}
            />
