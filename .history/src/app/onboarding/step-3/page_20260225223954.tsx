...
export default function Step3() {
  ...
  const platformCards = [
    ...
    },
    {
      key: "whatsapp",
      name: "WhatsApp",
      desc: "Business messaging via Twilio",
      icon: <Phone className="h-7 w-7 text-success" />,
      iconBg: "bg-success/10",
      connected: isWhatsAppConnected,
      onConnect: undefined, // custom UI below
    },
    {
      key: "discord",
      name: "Discord",
              ...
              {p.key === "whatsapp" && !p.connected && (
                <div className="mt-4 flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 border border-border rounded-lg px-3 py-2 text-sm bg-card text-foreground focus:outline-none focus:border-success/40 transition-all"
                  />
                  <Button
                    size="sm"
                    disabled={!whatsAppPhone.trim() || isConnecting === "whatsapp"}
                    onClick={(e) => {
                      e.stopPropagation();
