...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      // Ensure oauth_tokens record exists (settings page checks this for "Connected" status)
      await convex.mutation(api.oauth.storeTokens, {
        userId: user._id,
        platform: "whatsapp",
        accessToken: "twilio-managed",
        platformUserId: normalizedPhone,
      });
    ...
    // (WhatsApp uses Twilio credentials from env vars rather than per-user OAuth,
    //  so we store a marker record so the settings UI shows "Connected")
    await convex.mutation(api.oauth.storeTokens, {
      userId: user._id,
      platform: "whatsapp",
