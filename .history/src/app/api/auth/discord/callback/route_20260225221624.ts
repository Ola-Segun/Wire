  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL("/dashboard/settings?error=discord_denied", req.url)
      );
    }

    if (!code) {
      return NextResponse.redirect(
        new URL("/dashboard/settings?error=discord_no_code", req.url)
      );
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(
        new URL("/dashboard/settings?error=discord_token_failed", req.url)
      );
    }
    ...
    if (!user) {
      return NextResponse.redirect(
        new URL("/dashboard/settings?error=user_not_found", req.url)
      );
    }
    ...
    try {
      await convex.mutation(api.onboarding.addPlatform, {
        userId: user._id,
        platform: "discord",
      });
    } catch {
    ...
    return NextResponse.redirect(
      new URL("/dashboard/settings?success=discord_connected", req.url)
    );
  } catch (err) {
    console.error("Discord OAuth callback: error:", err);
    return NextResponse.redirect(
      new URL("/dashboard/settings?error=discord_error", req.url)
    );
  }
}
